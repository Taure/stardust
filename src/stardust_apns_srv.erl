...
-record(state,
        {con = undefined :: pid(), ping, url, port, token, ttl, team_id, p8_key, key_id}).
...
handle_call(Request, _From, State) ->
    ?WARNING("UNEXPECTED CALL: ~p State: ~p", [Request, State]),
    {reply, ok, State}.
...
handle_info(Info, State) ->
    ?WARNING("UNEXPECTED INFO: ~p State: ~p", [?MODULE, Info, State]),
    {noreply, State}.
...
strip_url(<<$=, $=>>, Acc) ->
    Acc;
strip_url(<<$/, T/binary>>, Acc) ->
    strip_url(T, <<Acc/binary, $_>>);
strip_url(<<$+, T/binary>>, Acc) ->
    strip_url(T, <<Acc/binary, $->>);
strip_url(<<H, T/binary>>, Acc) ->
    strip_url(T, <<Acc/binary, H>>).
